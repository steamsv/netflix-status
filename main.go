...
var hostname = flag.String("hostname", "hostname", "custom hostname")
...
func report(unlock bool) {
	...
	if resp, err := http.Get(uri); err == nil {
		fmt.Printf("reported result to %s, result: %s", uri, resp.Status)
	} else {
		fmt.Printf("reported result failed: %s", err)
	}
	reported = true
}
